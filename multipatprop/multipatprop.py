...
class System:
    ...
    def get_multipath(self, starting_number: int, receiver_diameter: float, max_reflections: int,
                      power_multiplier: float = 0.9) -> Multipath:
    ...
    def get_path(self, starting_vector: Vector, receiver_diameter: float, max_reflections: int,
                 power_multiplier: float = 0.9) -> Path | None:
...
class Interferer:
    ...
    @classmethod
    def blob(cls, position: Point, average_radius: float, number_points: int, smoothing_factor: float = 0,
             smoothing_iterations: int = 0) -> Interferer:
        ...
        for s in range(smoothing_iterations):
            for p in range(number_points):
                ...
                points[(p + 1) % number_points] = Point(
                    point_middle.x * (1 - smoothing_factor) + point_target.x * smoothing_factor,
                    point_middle.y * (1 - smoothing_factor) + point_target.y * smoothing_factor)
